...
def _parse_from_filename(path: str) -> tuple[Optional[str], Optional[int], Optional[int]]:
    """Parse (stid, year, month) from filename; fallback to parent folder for stid."""
...
def process_monthly_erosivity(
    ...
    erosivity_col: str = "rainfall erosivity ((MJ mm)/(ha hr))",  # <- default to your per-event column
    ...
    Writes one CSV per station with a complete year-month grid and returns a
    DataFrame of non-zero monthly sums.

    Inputs are typically produced by rainfall_erosivity.process_rainfall_erosivity:
      <input_dir>/<STID>/<YEAR>/<STID>_<YEAR>_<MONTH>.csv
    """
    ...
    for fp in files:
        ...
        if erosivity_col not in df.columns:
            # Not a per-event file (e.g., combined summary) — skip quietly
            # Uncomment to log: print(f"❌ {fp}: Missing erosivity column '{erosivity_col}'; skipping.")
            continue
        ...
        # --- Year/Month ---
        if (year_col and year_col in df.columns) and (month_col and month_col in df.columns):
            ...
            if y_from_name is None or m_from_name is None:
                # Can't determine year/month — skip this file
                # Uncomment to log: print(f"❌ {fp}: Cannot determine year/month; skipping.")
                continue
    ...
    if not all_rows:
        print("ℹ️ No valid per-event rows found to aggregate.")
        return pd.DataFrame()

    big = pd.concat(all_rows, ignore_index=True)

    # Group & sum to create sparse monthly totals
    monthly = (
    ...
    # Write one file per station with a complete month grid (fill missing months with 0)
